...
static void identifyQueryResponseCb(ZCL_Addressing_t *addressing, ZCL_IdentifyQueryResponse_t *payload)
{
  if (isEzModeInProgress() && !ezModeMem.isBindingStarted)
  {
    ...
    ezModeMem.appBindReq = getDeviceBindRequest();
    ezModeMem.appBindReq->nwkAddrOfInterest = addressing->addr.shortAddress;
...
static void bindNextCluster(void)
{
  AppBindReq_t *appBindReq = ezModeMem.appBindReq;
  ...
  if (ezModeMem.clusterNumber == appBindReq->remoteClientsCnt)
  {
    ...
    identifySendUpdateCommissioningState(APS_SHORT_ADDRESS, ezModeMem.appBindReq->nwkAddrOfInterest,
