...
static ZclCommand_t * zclGetNextCommand(ZclCommand_t *command);
...
static void zclReportOnChangeIfNeeded(ZclAttribute_t *attr, ZCL_DataTypeDescriptor_t *desc)
{
  if ((attr->properties & ZCL_REPORTABLE_ATTRIBUTE) && isReportingPermitted(attr) && isOnChangeReportingNeeded(attr))
  {
    uint8_t *reportCounterPtr = (uint8_t *)attr + ATTRIBUTE_ID_SIZE + ATTRIBUTE_TYPE_SIZE +
                                sizeof(uint8_t) + desc->length; // properties + value
    ZclReportableAttributeTail_t *attrTail = (ZclReportableAttributeTail_t *)reportCounterPtr;

    attr->properties |= ZCL_ON_CHANGE_REPORT;
    if ((0 == attrTail->maxReportInterval) || (attrTail->minReportInterval <= attrTail->reportCounter))
      zclStartReportTimer(attr, 0, false);
    else
      zclStartReportTimer(attr, attrTail->minReportInterval - attrTail->reportCounter, true);
  }
}
...
ZCL_Cluster_t *ZCL_GetCluster(Endpoint_t endpointId, ClusterId_t clusterId, uint8_t clusterSide)
{
  ...
  {
    sysAssert(false, ZCLGETCLUSTER_0);
    return NULL;
  }
  if (clusterCounter)
...
static ZclStructureElement_t *zclObtainStructureElement(const ZclStructure_t *st, uint16_t idx)
{
  ...
  if (idx > st->cnt)
  {
    sysAssert(false, ZCL_UNBOUNDED_READ);
    return NULL;
  }
  ...
  switch (typeId)
  {
    ...
    case ZCL_STRUCTURE_DATA_TYPE_ID:
      ...
      if (value)
      {
        ...
        length = (const uint8_t *)zclObtainStructureElement(st, st->cnt) - value;
      }
      break;

    default:
      sysAssert(false, UNKNOWN_DAT_TYPE_DESCR);
      break;
  }
