...
******************************************************************************/
static void zclExecuteRequest(ZCL_Request_t *req, ZclMmBuffer_t *buf);
...
static void zclExecuteRequest(ZCL_Request_t *req, ZclMmBuffer_t *buf)
{
  ...
  sysAssert(cluster != NULL, ZCL_UNDEFINED_CLUSTER_IN_REQUEST);
...
void zclTaskHandler(void)
{
  ...
  while (NULL != (req = (ZCL_Request_t *)getQueueElem(&zclModuleMem.requestQueue)))
  {
    ...
    if (req->requestLength > bufferSize)
    {
      sysAssert(false, ZCL_BUFFER_SIZE_IS_TOO_LOW);
      if (req->ZCL_Notify)
      {
...
bool zclReadReportingConfigurationHandler(APS_DataInd_t *apsDataInd, ZclFrameDescriptor_t *frameDescriptor)
{
  ...
  CS_ReadParameter(CS_ZCL_BUFFER_SIZE_ID, (void *)&maxFrameSize);
  ...
  while (0 != inPayloadLength)
  {
    ...
    if (maxFrameSize >= outPayloadLength + predictedRecordLength)
    {
      resp->status = status;
...
bool zclConfigureReportingHandler(APS_DataInd_t *apsDataInd, ZclFrameDescriptor_t *frameDescriptor)
{
  ...
  ZCL_DataTypeDescriptor_t dataTypeDescriptor;
  uint8_t maxFrameSize;
  uint16_t minTimeInterval = 0xFFFFu;
  ...
  while (0 != inPayloadLength)
  {
    ...
    if (maxFrameSize >= outPayloadLength + sizeof(ZCL_ConfigureReportingResp_t))
    {
      if (ZCL_FRAME_CONTROL_DIRECTION_CLIENT_TO_SERVER == req->direction)
      {
          ...
          else
          {
            if (predictedRecordLength <= inPayloadLength)
            {
              if (attribute->type == req->attributeType)
              {
                ...
                if (0 != req->maxReportingInterval)
                  minTimeInterval = MIN(minTimeInterval, req->maxReportingInterval);
  ...
  if (0xFFFFu != minTimeInterval)
    zclStartReportTimer(attribute, minTimeInterval, false);
  ...
  while (NULL != (endpoint = zclNextEndpoint(endpoint)))
  {
    ...
    while (clusterCounter--)
    {
      ...
      for (uint8_t attrIndex = 0; attrIndex < cluster->attributesAmount; attrIndex++)
      {
        ...
        {
          attrPtr += sizeof(ZclReportableAttributeTail_t) + attrLength + sizeof(ZCL_ReportTime_t) + attrLength; // tail + reportableChange + timeoutPeriod + value
          continue;
        }
        ...
        else if (tail->reportCounter < tail->maxReportInterval)
        {
...
/*************************************************************************//**
  \brief Adjusts ZCL report timer's interval. Sets interval to the closest
    reporting time among all times.

  \param[in] attribute     - pointer to attribute.
  \param[in] timeout       - new timeout, seconds.
  \param[in] adjustTimeout - substract time passed from the last report from
  timeout if true, otherwise false
 *****************************************************************************/
void zclStartReportTimer(ZclAttribute_t *attribute, uint32_t timeout, bool adjustTimeout)
{
  ZCL_DeviceEndpoint_t *endpoint = NULL;
  uint32_t  minTimeout = ~0ul;
  uint32_t curTime     = halGetTimeOfAppTimer();
  uint8_t attrLength   = ZCL_GetAttributeLength(attribute->type, (uint8_t *)&attribute->value);
  uint8_t *reportCounterPtr = (uint8_t *)attribute + ATTRIBUTE_ID_SIZE + ATTRIBUTE_TYPE_SIZE +
                                      sizeof(uint8_t) + attrLength; // properties + value
  ZclReportableAttributeTail_t *tail = (ZclReportableAttributeTail_t *)reportCounterPtr;

  if (adjustTimeout)
  {
    timeout = (timeout > (curTime - zclModuleMem.reportTimer.timer.service.sysTimeLabel) / 1000 ?
      timeout - (curTime - zclModuleMem.reportTimer.timer.service.sysTimeLabel) / 1000 : 0);
    tail->reportCounter += (tail->minReportInterval - timeout);
  }

  // the only active reportable attribute
  if (SYS_TIMER_STOPPED == zclModuleMem.reportTimer.state)
  {
    SYS_InitTimer(&zclModuleMem.reportTimer, TIMER_ONE_SHOT_MODE, timeout * 1000, zclReportTimerFired);
    SYS_StartTimer(&zclModuleMem.reportTimer);
    return;
  }
  ...
  while (NULL != (endpoint = zclNextEndpoint(endpoint)))
  {
    ...
    for (uint8_t clusterIndex = 0; clusterIndex < endpoint->simpleDescriptor.AppInClustersCount; clusterIndex++)
    {
      ...
      for (uint8_t attrIndex = 0; attrIndex < cluster->attributesAmount; attrIndex++)
      {
        if ((attribute != attr) && (attr->properties & ZCL_REPORTABLE_ATTRIBUTE) && isReportingPermitted(attr))
        {
