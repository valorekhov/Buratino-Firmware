...
// Macros for debug needs - to implement static assert, checking sizeof(ZclFrameHeader_t).
#define  ZCL_FRAME_HEADER_SIZE  5
...
static uint8_t getElementLength(const ZCL_NextElement_t *element, bool formRequest)
{
  ...
  CS_ReadParameter(CS_ZCL_BUFFER_SIZE_ID, (void *)&result);
                                          ...
                                          bool undivided)
{
  ...
  while (phase)
  {
    ...
    while (payloadLength)
    {
      ...
      {
        sysAssert(0, ZCLPARSER_FORMWRITEATTRIBUTERESPONSE0);
        return 0;
      }
                                          ...
                                          const ZclFrameDescriptor_t *reqFrame)
{
  ...
  for (uint8_t reqLen = reqFrame->payloadLength; reqLen; reqLen -= sizeof(ZCL_ReadAttributeReq_t), req++)
  {
    ...
    {
      sysAssert(0, ZCLPARSER_FORMREADATTRIBUTERESPONSE0);
      break;
    }
...
static bool isSupportedManufacturerSpecCode(uint16_t  manufacturerCode)
{
  if (ATMEL_ID == manufacturerCode ||
      MESHNETICS_ID == manufacturerCode)
    return true;
