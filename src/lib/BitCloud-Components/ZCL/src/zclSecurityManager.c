...
{
  sysAssert(req, ZCL_STARTSECURITY_NULLREQUEST);
  sysAssert(req->ZCL_StartSecurityConf, ZCL_STARTSECURITY_NULLCONFIRM);
  smStartReq = req;
...
static void smMakeStartConf(void)
{
  if (smStartReq)
  {
    ...
    smStartReq = NULL;
    sysAssert(cf, ZCL_STARTSECURITY_NULLCALLBACK0);
    cf(conf);
  }
}
