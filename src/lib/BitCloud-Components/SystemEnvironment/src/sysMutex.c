/**************************************************************************//**
  ...
  \internal
   ...
   Last change:
    $Id: sysMutex.c 25824 2013-12-10 08:44:12Z unithra.c $
 ******************************************************************************/
/******************************************************************************
                               Includes section
 ******************************************************************************/
...
bool SYS_MutexLock(SYS_Mutex_t *const mutex, SYS_MutexOwner_t *const owner)
{
  ...
  sysAssert(NULL != mutex && NULL != owner, SYSMUTEX_MUTEXLOCK0);
...
bool SYS_MutexUnlock(SYS_Mutex_t *const mutex, SYS_MutexOwner_t *const owner)
{
  ...
  sysAssert(NULL != mutex && NULL != owner, SYSMUTEX_MUTEXUNLOCK0);

  qelem = getQueueElem(mutex);
  if (NULL == qelem)
  {
    sysAssert(false, SYSMUTEX_MUTEXUNLOCK1);
    return false;
  }
  ...
  {
    sysAssert(false, SYSMUTEX_MUTEXUNLOCK2);
    return false;
  }
  else
  ...
  {
    currentOwner = GET_PARENT_BY_FIELD(SYS_MutexOwner_t, service.qelem, qelem);
    sysAssert(currentOwner->SYS_MutexLockConf, SYSMUTEX_MUTEXUNLOCK3);
...
{
  sysAssert(NULL != mutex && NULL != owner, SYSMUTEX_ISMUTEXLOCKED0);
