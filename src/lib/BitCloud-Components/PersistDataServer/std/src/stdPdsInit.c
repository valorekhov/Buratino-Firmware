/**************************************************************************//**
  ...
  \internal
   ...
   Last change:
    $Id: stdPdsInit.c 25824 2013-12-10 08:44:12Z unithra.c $
 ******************************************************************************/
...
void PDS_Init(void)
{
  ...
  sysAssert(0 == (PDS_FF_SIZE % sizeof(PDS_FileDescr_t)), PDSINIT_PDSINIT0);
  pdsMemory()->ffSize |= (PDS_FF_SIZE / sizeof(PDS_FileDescr_t));
  sysAssert(0 == (PDS_FD_SIZE % sizeof(PDS_DirDescr_t)), PDSINIT_PDSINIT1);
...
static void pdsInitFileOffsetTable(void)
{
  ...
  for (memoryId = 0; memoryId < PDS_MEM_IDS_AMOUNT; memoryId++)
    if (PDS_MEM_MASK_IS_BIT_SET(pdsMemory()->allowedForStoring, memoryId))
    {
      ...
      sysAssert(fileDescrRec, PDSINIT_PDSINIT2);
