...
#ifndef _MAC2_
RAM_PARAMETER(CS_MAX_NEIGHBOR_ROUTE_COST_ID, 0x04, csNIB.maxNeighborRouteCost)
RAM_PARAMETER(CS_MAX_LINK_ROUTE_COST_ID, 0x05, csNIB.maxLinkRouteCost)
RAM_PARAMETER(CS_NWK_EXT_PANID_ID, 0x06, csNIB.extendedPanId)
RAM_PARAMETER(CS_NWK_ADDR_ID, 0x07, csNIB.networkAddress)
RAM_PARAMETER(CS_NWK_PARENT_ADDR_ID, 0x08, csNIB.parentNetworkAddress)
RAM_PARAMETER(CS_NWK_DEPTH_ID, 0x09, csNIB.depth)
RAM_PARAMETER(CS_NWK_UNIQUE_ADDR_ID, 0x0A, csNIB.uniqueAddr)
RAM_PARAMETER(CS_NWK_LEAVE_REQ_ALLOWED_ID, 0x0B, csNIB.leaveReqAllowed)
RAM_PARAMETER(CS_CHANNEL_PAGE_ID, 0x0C, csNIB.channelPage)
RAM_PARAMETER(CS_NWK_USE_MULTICAST_ID, 0x0D, csSIB.csNwkUseMulticast)
RAM_PARAMETER(CS_NWK_MAX_LINK_STATUS_FAILURES_ID, 0x0E, csSIB.csNwkMaxLinkStatusFailures)
RAM_PARAMETER(CS_NWK_END_DEVICE_MAX_FAILURES_ID, 0x0F, csSIB.csNwkEndDeviceMaxFailures)
RAM_PARAMETER(CS_NWK_LOGICAL_CHANNEL_ID, 0x10, csSIB.csNwkLogicalChannel)
RAM_PARAMETER(CS_NWK_PANID_ID, 0x11, csSIB.csNwkPanid)
RAM_PARAMETER(CS_NWK_PREDEFINED_PANID_ID, 0x12, csSIB.csNwkPredefinedPanid)
RAM_PARAMETER(CS_ADDRESS_ASSIGNMENT_METHOD_ID, 0x13, csSIB.csAddressAssignmentMethod)
RAM_PARAMETER(CS_PROTOCOL_VERSION_ID, 0x14, csNIB.protocolVersion)
RAM_PARAMETER(CS_STACK_PROFILE_ID, 0x15, csNIB.stackProfile)
RAM_PARAMETER(CS_SCAN_DURATION_ID, 0x16, csZIB.scanDuration)
RAM_PARAMETER(CS_PERMIT_DURATION_ID, 0x17, csZIB.permitJoinDuration)
RAM_PARAMETER(CS_EXT_PANID_ID, 0x18, csSIB.csExtPANID)
RAM_PARAMETER(CS_CHANNEL_MASK_ID, 0x19, csZIB.channelMask)
