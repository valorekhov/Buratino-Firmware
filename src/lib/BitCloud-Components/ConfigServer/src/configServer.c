...
#if !defined(ZAPPSI_HOST) || defined(ZCL_SUPPORT)
...
void CS_GetMemory(CS_MemoryItemId_t memoryId, void **memoryPtr)
{
  ...
  sysAssert((memoryId & CS_TYPE_MASK) == CS_MEM_PARAM_TYPE, CS_GET_MEM0);
...
void CS_ReadParameter(CS_MemoryItemId_t parameterId, void *parameterValue)
{
  ...
  */
  ...
  switch (parameterId)
  {
    ...
    case CS_ZCL_OTAU_IMAGE_PAGE_REQUEST_RESPONSE_SPACING_ID:
...
static void csReadInternalParameter(CS_MemoryItemId_t parameterId, void *parameterValue)
{
  ...
  sysAssert(parameterValue, CS_READ_PARAM0);
  ...
  const void *parameterValue)
{
  ...
  sysAssert(parameterValue, CS_WRITE_PARAM0);
  sysAssert((parameterId & CS_TYPE_MASK) == CS_RAM_PARAM_TYPE, CS_WRITE_PARAM1);
